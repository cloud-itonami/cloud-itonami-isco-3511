(ns ictops.actor
  "ICTOperationsTechniciansActor — the ISCO-08 3511 community ictops actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one ictops operation request
  ...
  The unconditional invariant: the ICTOperationsTechniciansAdvisor can never
  directly commit a record the ICTOperationsTechniciansGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [ictops.advisor :as advisor]
            [ictops.governor :as governor]
            [ictops.store :as store]))

(defn build-graph
  "Build a compiled ICTOperationsTechniciansActor graph. `store` implements
  `ictops.store/Store`. `advisor` implements
  `ictops.advisor/Advisor` (defaults to `mock-advisor`).
